create table if not exists basic_user (id bigint not null auto_increment, active bit not null, email varchar(50) not null, first_name varchar(15) not null, last_name varchar(20) not null, login varchar(25) not null unique,password_ varchar(40) not null, phone_number varchar(12) not null, primary key (id), unique (email, login));

create table if not exists class_info (id bigint not null auto_increment, description varchar(255), when_ bigint, where_ varchar(255), level_id bigint not null, primary key (id));

create table if not exists class_student (student_id bigint not null, class_id bigint not null, primary key (student_id, class_id));

create table if not exists detailed_info (id bigint not null auto_increment, city varchar(40) not null, country varchar(40) not null, street varchar(40) not null, zip_code varchar(6) not null, date_joined bigint not null, date_of_birth bigint not null, pin varchar(255) unique, primary key (id), unique (pin));

create table if not exists grade (id bigint not null auto_increment, comment_ varchar(255), percentage tinyint not null, weight tinyint not null, student_id bigint not null, test_id bigint, primary key (id));

create table if not exists level (id bigint not null auto_increment, name_ varchar(255), primary key (id));

create table if not exists parent (id bigint not null, primary key (id));

create table if not exists parent_student (parent_id bigint not null, student_id bigint not null, primary key (parent_id, student_id), unique (student_id));

create table if not exists student (id bigint not null, info_id bigint not null unique, level_id bigint not null, primary key (id));

create table if not exists teacher ( avatar longblob, id bigint not null, info_id bigint not null unique, primary key (id) );

create table if not exists teacher_class ( teacher_id bigint not null, class_id bigint not null, primary key (teacher_id, class_id) );

create table if not exists test ( id bigint not null auto_increment, description varchar(255), when_ bigint not null, class_id bigint, primary key (id) );
